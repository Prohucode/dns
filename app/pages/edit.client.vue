<template>
	<Card line>
		<template #header>
			<b class="pa-16">Товары по заявке {{ id }}</b>
		</template>
			...
			<Table v-if="products.length">
				...
				<tbody>
					<tr v-for="product in products" :key="product.name">
						<td>{{ product.name }}</td>
						<td>
							<Input v-model="product.amount" placeholder="Количество"
								:class="{ error: errors[product.id]?.amount }" />
						</td>
						<td>
							<Input v-model="product.price" :decimals="2" placeholder="Цена"
								:class="{ error: errors[product.id]?.price }" />
						</td>
						<td>
							<Select v-model="product.color" :options="colors" placeholder="Выберите цвет"
...
// ошибки по id товара
const errors = ref<Record<number, { amount?: boolean; price?: boolean; color?: boolean }>>({})
